...
namespace Business_Class
{
    ...
    {
        public static string filepath = "F:\\SECOND SEMESTER\\OOP\\Programming Day(PD)\\WEEK 2\\Business_Class - Copy\\data.txt";
        ...
        static void Store_Data(Class1[] s, int usercount)
        {
            StreamWriter filename = new StreamWriter(filepath);
            for (int i = 0; i < usercount; i++)
            {
                filename.Write($"{s[i].userName},{s[i].userpassword},{s[i].userrole}");
                if (i < usercount - 1)
                {
                    filename.Write("\n");
                }
        ...
        static void Read_Data(Class1[] s, ref int usercount)
        {
            int index = 0;
            ...
            using (StreamReader filename = new StreamReader(filepath))
            {
                string line = "";
                while ((line = filename.ReadLine()) != null)
                {
                    string[] parts = line.Split(',');
                    if (usercount < s.Length)
                    {
                        s[usercount].userName = parts[0];
                        s[usercount].userpassword = parts[1];
                        s[usercount].userrole = parts[2];
                        usercount++;
                        //count++;
                        }
                    else
                        {
                            // arrays are full, handle this situation accordingly
                            Console.WriteLine("arrays are full. cannot load more data.");
                            break; // exit the loop
                        }
                    }
                filename.Close();
       ...
       static void ToUpper(ref string input)
        {
